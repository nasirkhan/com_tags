...
/**
 * @package     Joomla Tags Component
 * @subpackage  com_tags
 *
 * @copyright   (C) 2022 Nasir Khan Saikat
...
<div class="com-tags__items">
    <div class="row mb-4">
        <div class="col">
            <form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="my-4">
    ...
    <?php else : ?>
        <div class="row mb-4">
            <?php foreach ($this->items as $i => $item) : ?>
                <div class="col-12 col-sm-4 <?php ($item->core_state == 0) ? " bg-warning " : "";  ?>">
                    <div class="card mb-4">
                        ...
                        <?php elseif ($item->type_alias === 'com_content.article') : ?>
                            <?php
                            $image_src = '';
                            $image_alt = $this->escape($item->core_title);

                            $core_params = json_decode($item->core_params);
                            if (isset($core_params->helix_ultimate_image)) {
                                $image_src = $core_params->helix_ultimate_image;
                            } else {
                                $image_src = '';
                            }
                            ...
                            if (empty($image_src)) {
                                ...
                                $image_src = $core_images->image_intro;
                                $image_alt = (($core_images->image_intro_alt != "") ? $core_images->image_intro_alt . " | " : "") . $image_alt;
                            }
                            ?>

                            <a href="<?php echo Route::_($item->link); ?>">
                                <img src="<?php echo $image_src; ?>" alt="<?php echo $image_alt; ?>" class="card-img-top">
                            </a>
